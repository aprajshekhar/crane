...
class Solr(HTTPBackend):
    ...
    def search(self, query):
        ...
        filtered_results = itertools.ifilter(self._filter_result_v2_without_auth_check, results)
    ...
    def _filter_result_v2_without_auth_check(self, result):
        ...
        :param result:  one search result
        :type  result:  SearchResult
        :return:    True if either the repository is known or if check can be skipped else False
        :rtype:     bool
        """
        if result.should_filter:
            return super(Solr, self)._filter_result_v2_without_auth_check(result)
        else:
            return True
